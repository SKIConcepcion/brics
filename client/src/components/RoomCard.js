...
export default function RoomCard({ room, link }) {
    ...
    const slideClassName = "blue-slide";
  ...
  const podiumAmen = {
    ...
    //   ] //for testing only
      ...
      // }
      return ( 
        <div className="max-w-full md:max-w-md lg:max-w-xl flex flex-col h-full"> 
            <Carousel images={room.pictures} slideClassName={slideClassName}/>
            <div className="-mt-10 w-full h-xl px-2 py-1 flex flex-col flex-grow items-start gap-1 border border-gray-300 rounded-lg w-400">
                <div className="flex self-stretch mt-10 justify-between flex-grow items-center inline-flex">
                    <div className="text-left text-gray-700 font-bold text-lg">{room.name}</div>
                    <div className="text-left text-sm text-gray-700 px-3">Seating Capacity: {room.seatingCapacity}</div>
                </div>
                <div className="font-medium text-md leading-normal text-gray-700">₱{room.reservationFee}</div>
                <div className="self-stretch h-0.5 bg-gray-200 rounded-lg"></div>
                <div className="flex flex-wrap self-stretch py-1 px-0 justify-between items-center inline-flex">
                    <div className="h-7 justify-between items-center flex">
                        <div className="justify-center items-center gap-2 flex">
                            {room.computerCount > 0 && (
                                <>
                                    {computerAmen.icon}
                                    <div className="text-black text-sm font-medium">{room.computerCount}</div>
                                </>
                            )}
                    ...
                    { user === null || user === undefined ? (
                       <button 
                       onClick={() => navigate(link, { state: { roomId: room._id} })} //pass roomId to the next page
                       className={` items-center rounded-lg gap-x-1 bg-blue-500 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none px-6 py-1`}>
                       Inquire
                   </button>           
                    ) : (
                      <button 
